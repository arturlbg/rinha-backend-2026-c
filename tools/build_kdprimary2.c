...
static void usage(void) {
    fprintf(stderr, "usage: build_kdprimary2 --index <index.bin> --output <kdprimary2.bin> [--leaf-size N]\n");
}
...
int main(int argc, char **argv) {
    ...
    if (ivf8_index_open(index_path, &index, err, sizeof(err)) != 0) {
        fprintf(stderr, "build_kdprimary2: %s\n", err);
        return 1;
    }
    ...
    printf("leaf_size=%u\n", leaf_size);
    printf("node_size=%zu\n", sizeof(KdPrimary2Node));

    KdPrimary2Build build;
    uint64_t start = now_ns();
    if (kdprimary2_build_from_ivf8(&build, &index, leaf_size, err, sizeof(err)) != 0) {
        fprintf(stderr, "build_kdprimary2: %s\n", err);
    ...
    start = now_ns();
    if (kdprimary2_save(&build, output_path, err, sizeof(err)) != 0) {
        fprintf(stderr, "build_kdprimary2: %s\n", err);
    ...
    size_t file_bytes = kdprimary2_expected_file_bytes(build.node_count, build.block_count);
    printf("nodes=%u\n", build.node_count);
    printf("blocks=%u\n", build.block_count);
    printf("root=%u\n", build.root);
    printf("points=%u\n", build.count);
    ...
    printf("build_memory_bytes_estimate=%zu\n", kdprimary2_build_memory_bytes(&build));
    printf("build_memory_mib_estimate=%.2f\n", (double)kdprimary2_build_memory_bytes(&build) / 1048576.0);
