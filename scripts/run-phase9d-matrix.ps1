...
$ResultRoot = Join-Path $Root "tmp\results\phase9d-$Stamp"
...
$image = "rinha-c-phase9d:local"

$cases = @(
    @{
        Name = "direct_blocking_0475"
        Compose = $direct
        Env = @{
            RINHA_IMAGE = $image
...
if ($CaseFilter -ne "") {
    ...
    if ($cases.Count -eq 0) {
        throw "no Phase 9D cases matched CaseFilter=$CaseFilter"
    }
}
...
if ($MetricsDiagnostic) {
    $diag = @{
        Name = "direct_epoll_0475_metrics"
        Compose = $direct
        Env = @{
            RINHA_IMAGE = $image
            RINHA_DIRECT_CPUS = "0.475"
...
Write-Host "phase9d_result_root=$ResultRoot"
