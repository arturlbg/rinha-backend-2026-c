...
$ResultRoot = Join-Path $Root "tmp\results\phase10a-$Stamp"
...
function Read-RunSummary([string]$RunDir, [hashtable]$Case, [int]$Run) {
    ...
    return [PSCustomObject]@{
        name = $Case.Name
        run = $Run
        lb_cpus = $Case.Env.LB_CPUS
        api_cpus = $Case.Env.API_CPUS
        lb_memory = $Case.Env.LB_MEMORY
        api_memory = $Case.Env.API_MEMORY
        p99 = $result.p99
...
$baseEnv = @{
    ...
    RINHA_LB_CONNECT_RETRY_MS = "25"
    RINHA_LB_STARTUP_TIMEOUT_MS = "10000"
}

$cases = @(
    @{ Name = "baseline"; Env = $baseEnv + @{ LB_CPUS = "0.05"; API_CPUS = "0.475"; LB_MEMORY = "16m"; API_MEMORY = "167m" } },
    @{ Name = "api_heavy_1"; Env = $baseEnv + @{ LB_CPUS = "0.03"; API_CPUS = "0.485"; LB_MEMORY = "12m"; API_MEMORY = "169m" } },
    @{ Name = "api_heavy_2"; Env = $baseEnv + @{ LB_CPUS = "0.02"; API_CPUS = "0.49"; LB_MEMORY = "12m"; API_MEMORY = "169m" } },
    @{ Name = "lb_heavy"; Env = $baseEnv + @{ LB_CPUS = "0.10"; API_CPUS = "0.45"; LB_MEMORY = "16m"; API_MEMORY = "167m" } },
    @{ Name = "middle"; Env = $baseEnv + @{ LB_CPUS = "0.04"; API_CPUS = "0.48"; LB_MEMORY = "12m"; API_MEMORY = "169m" } }
)

if ($CaseFilter -ne "") {
    $cases = @($cases | Where-Object { $_.Name -like $CaseFilter })
    if ($cases.Count -eq 0) {
        throw "no Phase 10A cases matched CaseFilter=$CaseFilter"
    }
}
...
Write-Host "phase10a_result_root=$ResultRoot"
